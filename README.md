...
- 読み上げ速度変更（`0.5` 〜 `2.0`）
- 最大読み上げ文字数変更（`10` 〜 `500`）
- 送信者名を読むかどうかの切り替え（初期値: 読む、同一送信者が60秒未満で連投した場合は名前省略）
- VC 入退室アナウンス
- URL・メンションなどの読み上げ前テキスト前処理
- 設定の再起動後復元（話者/スタイル/速度/文字数/送信者名読み上げ/読み上げチャンネル）
...
`/style` と `/styleall` は、旧設定との互換のために `VOICEVOX:ずんだもん` のスタイルIDを基準にしたプリセットです（現在は複数話者に対応しています）。
...
- **音声クレジット表記**: 使用する話者の各音声ライブラリ規約に従って表記してください
